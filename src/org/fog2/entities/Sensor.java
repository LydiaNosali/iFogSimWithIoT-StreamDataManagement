...
//		LatencyStats.add_Overall_write_Letency(LatencyStats.getOverall_write_Latency()+delay*nb_Unit);
		...
		/*
		 * tab [0]: DataProd Id <----- tuple
		 * tab [1]: destinater service name
		 * tab [2]: edge
		 * tab [3]: list of response replicas
		 * tab [4]: reserved for the median replicas
		 * tab [5]: requestId
		 */
		...
		//*System.out.println(this.getName()+" send tuple to module:"+tuple.getDestModuleName().get(0));
		Log.writeInLogFile(this.getName()," send tuple to module:"+tuple.getDestModuleName().get(0));
		send(gatewayDeviceId, delay*nb_Unit, FogEvents.TUPLE_PROCESS,tab);
	}
