...
public class FogDevice extends PowerDatacenter {
	...
	public Map<String, Tuple> initial_cached_data = new HashMap<String, Tuple>();
	
	public double free_capacity;
	public double stored_IoT_size;
	...
	public FogDevice(
		...
		free_capacity = this.getHostList().get(0).getStorage();
		stored_IoT_size = 0;
	}
	...
	protected void processOtherEvent(SimEvent ev) {
		...
		case CloudSimTags.VM_DATACENTER_EVENT:
			...
			checkCloudletCompletion((Tuple) tab[5]);
			break;
		...
		case FogEvents.ADD_STREAM_TO_CACHE:
			processAdd_Stream_To_Cache(ev);
			break;
			
		case FogEvents.STREAM_INITIAL_STORAGE:
			processStream_initial_storage(ev);
			break;
			
		case FogEvents.CREATE_RETRIEVE_STREAM_DATA:
			processCreateRetrieveStreamData(ev);
			break;
			
		case FogEvents.SEND_STREAM_TO_CONSUMER:
			processSendStreamToConsumer(ev);
			break;
			
		case FogEvents.STREAM_PROCESS:
			processStreamProcess(ev);
			break;
	...
	public FogCache getFogCache() {
		FogDeviceCharacteristics fc = (FogDeviceCharacteristics) getDatacenterCharacteristics();
		FogCache cache = fc.getFogCache();
	...
	private void processStreamProcess(SimEvent ev) {
	...
	private void processSendStreamToConsumer(SimEvent ev) {
		// TODO Auto-generated method stub
		//**System.out.println(this.getName()+" process Send Stream To Consumer");
		
		Object tab [] = (Object []) ev.getData();
		
		String tupleType = (String) tab [0];
		int consumerId = (int) tab[1];
		
		//**System.out.println("tupleType:"+tupleType+" consumerId"+consumerId);
		
		FogCache cache = getFogCache();
				
		Tuple data = null;

		
		if (cache.cachedata.containsKey(tupleType)) {
			// data in cache
			data = cache.cachedata.get(tupleType);
			cache.retreive(tupleType);
			
		}else if (this.initial_cached_data.containsKey(tupleType)) {
			// data in initial emplacement
			data = this.initial_cached_data.get(tupleType);
			//**System.out.println("retrieve from initial emplacement at Node id :"+this.getId());
			
		}else {
			
			Log.writeInLogFile(this.getName(),"Error!, cache node" + this.getName() + " doesn't host any replica of " + tupleType);
			System.exit(0);
		}

		Tuple tupleSend = (Tuple) data.clone();

		float latency = BasisDelayMatrix.getFatestLink(getId(), consumerId);
	
		int nb_Unit = (int) Math.ceil(DataPlacement.size_streaming_data_transfert / DataPlacement.Basis_Exchange_Unit);
		
		//**System.out.println("nb_Unit:"+nb_Unit);

		send(consumerId, latency * nb_Unit, FogEvents.STREAM_PROCESS, tupleSend);
		//**System.out.println(this.getName()+" send FogEvents.STREAM_PROCESS to consumer Id : "+consumerId+" for tupletype :"+tupleType);
...
//		System.out.println("fogdev penality stream : "+PenalityStats.penalityStream(latency * nb_Unit));
//		System.out.println();
		PenalityStats.add_Overall_read_penalityStream(PenalityStats.penalityStream(latency * nb_Unit));
//		System.out.println("add stream penality in processSendStreamToConsumer");
		PenalityStats.add_penality_per_consumer(consumerId, tupleType, PenalityStats.penalityStream(latency * nb_Unit));


		// LCE
		//**System.out.println("Applying LCE for intermediary nodes");
		List<Integer> lceNodes = getLceNodes(this.getId(), consumerId);
		lceNodes.remove(new Integer(this.getId()));
				
		lceNodes.add(consumerId);

		//**System.out.println("LCE nodes:" + lceNodes.toString());

		for (int nodeId : lceNodes) {
			latency = BasisDelayMatrix.getFatestLink(getId(), nodeId);
			send(nodeId, latency * nb_Unit, FogEvents.ADD_STREAM_TO_CACHE, tupleSend);
			//**System.out.println("send FogEvents.ADD_STREAM_TO_CACHE to:" + nodeId + " for tuple:" + tupleSend.getTupleType()+"time:"+latency * nb_Unit);
		}
		
		LatencyStats.increment_Cache_Hit(this.getName());
//		System.exit(0);
//
//		//**System.out.println("press enter");
	...
	private void processCreateRetrieveStreamData(SimEvent ev) {
		// TODO Auto-generated method stub
		
		String tupleType = (String) ev.getData();
		//if the consumer is a HGW and the data is of type stream then send request to broker to ask producer to retrieve it
		if(this.getName().contains("HGW") && tupleType.contains("Stream")) {
			//**System.out.println(this.getName()+" send to broker FogEvents.RETRIEVE_STREAM_DATA for Stream: "+tupleType);
			send(2, 50, FogEvents.RETRIEVE_STREAM_DATA, tupleType);
			
		}else {
			System.out.println("error in process retrieve stream data");
			System.exit(0);
		}
...
//		//**String str = sc.nextLine();
	...
	private void processStream_initial_storage(SimEvent ev) {
		// TODO Auto-generated method stub
		
		Tuple tuple = (Tuple) ev.getData();
		
//		//store tuple in the DC - cache part
//		FogCache cache = ((FogDeviceCharacteristics) this.getDatacenterCharacteristics()).getFogCache();
//		
//		
//		//**System.out.println(this.getName()+" cache "+cache);
//		double fogNodeOverflowCapacity = this.getHost().getStorage() - stored_IoT_size;
//		cache.store(tuple, this.getId(), free_capacity, fogNodeOverflowCapacity);

		
		//add tuple to a map initial_cached_data
		//those tuple will not be considered when applying LRU
		this.initial_cached_data.put(tuple.getTupleType(), tuple);
		
		//**System.out.println(this.getName()+" add Stream_initial_storage: "+tuple.getTupleType());

	}

	private void processAdd_Stream_To_Cache(SimEvent ev) {
		// TODO Auto-generated method stub

        
		Tuple tuple = (Tuple) ev.getData();
		
		//**System.out.println(this.getName()+" processAdd_Stream_To_Cache for "+tuple.getTupleType());
		//**System.out.println(this.getName()+" current stored_IoT_size:"+stored_IoT_size);	
		//**System.out.println(this.getName()+" total storage capacity:"+this.getHost().getStorage());
		//**System.out.println(this.getName()+" total free capacity:"+this.free_capacity);
		
		Log.writeInLogFile(this.getName()," processAdd_Stream_To_Cache for "+tuple.getTupleType());
		Log.writeInLogFile(this.getName()," current stored_IoT_size:"+stored_IoT_size);	
		Log.writeInLogFile(this.getName()," total storage capacity:"+this.getHost().getStorage());
		Log.writeInLogFile(this.getName()," total free capacity:"+this.free_capacity);
		
		
		double fogNodeOverflowCapacity = this.getHost().getStorage() - stored_IoT_size;
		//**System.out.println("fogNodeOverflowCapacity:"+fogNodeOverflowCapacity);
		
		FogCache cache = ((FogDeviceCharacteristics) getDatacenterCharacteristics()).getFogCache();
		
		//**System.out.println(" cache total capacity:"+cache.capacity);
		//**System.out.println(" cache used capacity (with overflow if there is):"+cache.getCacheUsedCapacity());
		cache.printAllStoredStreams();
		
		double freeOverflowCapacity = cache.store(tuple, this.getId(), free_capacity, fogNodeOverflowCapacity);
		
		
		//**System.out.println("new freeOverflowCapacity = free_capacity = "+freeOverflowCapacity);
		
		//**System.out.println();
		
		this.free_capacity = freeOverflowCapacity;
...
//		}
		...
		for(Double time: times){
			...
			Log.writeInLogFile(this.getName(), "Send tuple arrival time:"+time+"\ttuple:"+tuple.toString());
	...
	private void processReturnLastVersion(SimEvent ev){
		...
		if(!lastVersionMap.containsKey(reqId)){
			...
			Log.writeInLogFile(this.getName()," Error!!! Last Version map should be contain request Id :"+reqId);
			System.exit(0);
		}
		...
		if(lastVersionList==null){
			...
			Log.writeInLogFile(this.getName()," Error!!! Last Version list is null for request Id :"+reqId);
			System.exit(0);
		}
		...
		if(nbLastVersion==lastVersionList.size()){
			...
			if(recentVersion > -1){
				...
				if (!lastVersionList.isEmpty()){
					...
					Log.writeInLogFile(this.getName()," send an event SEND_DATA_TO_CONSUMER to replics"+nearest+" tuple"+tupleType+" for consumer:"+ (String) tab[1]);
					...
					//Mise à jour de la pénalité
					PenalityStats.add_Overall_read_penalityIoT(PenalityStats.penalityIoT(delay ));
					PenalityStats.add_penality_per_consumer(consumerId, tupleType, PenalityStats.penalityIoT(delay));
					
					
				}else{
					//**System.out.println(this.getName()+" Error!!! recent Last Version list is null for request Id :"+reqId);
					Log.writeInLogFile(this.getName()," Error!!! recent Last Version list is null for request Id :"+reqId);
					System.exit(0);
				...
				Log.writeInLogFile(this.getName(), " median doesn't retreive any host that stores the replica of "+tupleType);
				Log.writeInLogFile(this.getName(), "Consumer module:"+(String)tab[1]+" will be not served for:"+tupleType);
				Log.writeInLogFile(this.getName()," Send to broker  Non retrieve Tuple for edge:"+((AppEdge) tab[2]).toString());
				...
				PenalityStats.add_penality_per_consumer(2, tupleType, PenalityStats.penalityIoT(latency));
...
//		//**String str = sc.nextLine();
	...
	private void processGetLastVersion(SimEvent ev) {
		...
		Log.writeInLogFile(this.getName(), " Send to median:"+ev.getSource()+" return last version for tupleType:"+edge.getTupleType());
		...
		if (!this.storedData.containsKey(tupleType)) {
			...
			Log.writeInLogFile(this.getName(), "Error!, node"+this.getName()+" doesn't host any replica of "+tupleType);
			System.exit(0);
		...
		if(this.storedData.get(tupleType) == null){
			...
			Log.writeInLogFile(this.getName()," there is no stored replica for "+tupleType+" in this node"+this.getName());
			...
			Log.writeInLogFile(this.getName(), " last version:"+tuple.getTupleVersion());
		}
...
//		//**String str = sc.nextLine();
	...
	private void processVersionExchange(SimEvent ev) {
		...
		for(int rep : listOfResponseReplicas){
			...
			Log.writeInLogFile(this.getName(), " Send to replica in:"+rep+" event for get last version for tupleType:"+edge.getTupleType());
...
//		//**System.out.println("consumer service = "+(String) tab[1]);
//		//**System.out.println("consumer node id = "+(int) tab[0]);
//		
		if (!this.storedData.containsKey(tupleType)) {
		...
		if (this.storedData.get(tupleType) == null) {
			...
			Log.writeInLogFile(this.getName(), "Node"+this.getName()+" doesn't host any replica of "+tupleType);
			Log.writeInLogFile(this.getName(), "Consumer module:"+(String)tab[1]+" will be not served for:"+tupleType+" by node:"+this.getName());

			Log.writeInLogFile(this.getName()," Send to broker  Non stored Tuple for edge:"+((AppEdge) tab[2]).toString());
			sendNow(2, FogEvents.NON_TUPLE_RETRIEVE, tab);
...
//			PenalityStats.add_penality_per_consumer((int) tab[0], tupleType, PenalityStats.penalityIoT(latency));
//			PenalityStats.add_Overall_read_penalityIoT(PenalityStats.penalityIoT(latency));
//			PenalityStats.add_Overall_penalityIoT(PenalityStats.penalityIoT(latency));	
			...
			Log.writeInLogFile(this.getName(),"555555555555555555555555555");
			Log.writeInLogFile(this.getName(),edge.toString());
			Log.writeInLogFile(this.getName(),"consumer service = "+(String) tab[1]);
			Log.writeInLogFile(this.getName(),"consumer node id = "+(int) tab[0]);
			...
			int nb_Unit = (int) Math.ceil(DataPlacement.HGW_TUPLE_FILE_SIZE / DataPlacement.Basis_Exchange_Unit);
			
			////*//**System.out.println("tupleSend:"+tupleSend.toString());
			Log.writeInLogFile(this.getName(), "Send replica for process:"+tupleSend.toString()+"\n to consumer node :"+destDevId+" consumer module:"+dest.get(0));
			...
			PenalityStats.add_penality_per_consumer(destDevId, tupleType, PenalityStats.penalityIoT(latency*nb_Unit));
			PenalityStats.add_Overall_read_penalityIoT(PenalityStats.penalityIoT(latency*nb_Unit));
...
//			//**String str = sc.nextLine();
	...
	private List<Integer> getLceNodes(int id, int destDevId) {
		// TODO Auto-generated method stub
		
		//**System.out.println("Get LCE nodes between src:"+id+" and dest:"+destDevId);
		int src;
		List<Integer> lceNodes = new ArrayList<Integer>();
		
		while(id!= destDevId) {
			////**System.out.println("new destDevId:"+destDevId);
			src = BasisDelayMatrix.mFlowMatrix[id-3][destDevId-3] + 3;
			lceNodes.add(src);
			destDevId = src;
		}
...
//		tab[3] = timeList;
		...
		AppEdge edge = (AppEdge) tab[2];
		List<Double> consumptiontimes = (List<Double>) tab[3];
		

		Log.writeInLogFile(this.getName(),"edge: "+edge.toString());
		Log.writeInLogFile(this.getName(),"consusmer: "+consumer);
		Log.writeInLogFile(this.getName(),"consumerNodeId: "+consumerNodeId);
	...
	private void processInitializeConsumption_estimation(SimEvent ev) {
		...
		Log.writeInLogFile(this.getName()," Send to broker Tuple retrieve for edge:"+((AppEdge) tab[2]).toString()+" ev time:"+(ev.eventTime()));
		...
		if(this.getName().startsWith("HGW")){
			...
			//Mise à jour de la pénalité
			PenalityStats.add_Overall_read_penalityIoT(PenalityStats.penalityIoT(50 ));
			PenalityStats.add_penality_per_consumer(this.getId(), edge.getTupleType(), PenalityStats.penalityIoT(50));
	...
	protected void processDelayTupleStorage(SimEvent ev){
			...
			if(CloudSim.broker.getAppliaction().getDataConsistencyProtocol().getUnlockedReplicasForWrite(tuple.getTupleType(), CloudSim.broker.getDataAllocation()).contains(this.getId())){
				...
				Log.writeInLogFile(this.getName(), "delayed Storage tuple:"+tuple.toString());
				...
				if(storedqueue != null){		
					if(storedqueue.iterator().next().getTupleVersion()> tuple.getTupleVersion()){			
						...
						Log.writeInLogFile(this.getName(), " Version rollback! this write will be not considred!");
						...
						if(this.storedData.get(tuple.getTupleType()).contains(tuple)){
							...
							Log.writeInLogFile(this.getName(), " delayed Tuple has been well stored:"+tuple.toString()+"\n");
							sendNow(2,  FogEvents.DELAY_TUPLE_STORAGE_ACK, tuple);
									
						}else {
							//**System.out.println(this.getName()+ " delayed Tuple does not been well stored:"+tuple.toString());
							Log.writeInLogFile(this.getName(), "delayed Tuple does not been well stored:"+tuple.toString());
							System.exit(0);
						}
					}				
				}
				...
				Log.writeInLogFile(this.getName(), " Send old tuple write:"+tuple.toString()+"\nto the non selected storage node:"+this.getId()+" in time event:"+String.valueOf(latency+ev.eventTime()));
...
//			//**String str = sc.nextLine();
	...
	private void processTupleStorage(SimEvent ev) {
		...
		//**System.out.println(this.getName()+" processTupleStorage for tuple:"+tuple.getTupleType());
		//*//**System.out.println();
		Log.writeInLogFile(this.getName(), "Storage tuple:"+tuple.toString());
...
//			//Log.writeInLogFile(this.getName(), "  Error! Storage tuple is unlocked replicas:"+tuple.toString());
//			System.exit(0);
//		}
		...
		if (this.storedData.get(tuple.getTupleType()) == null) {
			...
			Log.writeInLogFile(this.getName(), " this is the first stored tuple:"+tuple.toString());
			

			
			if(free_capacity < tuple.getCloudletFileSize()) {
				// LRU must be applied
				if(!DataPlacement.use_overflow) {
					System.out.println("Error, overflow is disable, data should find its free storage space");
					System.exit(0);
				}
				
				this.free_capacity = cache.LruForIoTDataStorage(tuple, this.getId(),this.free_capacity);
			...
			this.free_capacity -= tuple.getCloudletFileSize();
			this.stored_IoT_size += tuple.getCloudletFileSize();
			
			if(free_capacity < 0.7 * this.getHost().getStorage()) {
				// Archive 
				this.free_capacity = archiveAllOldVersionOfAllTuples(this.free_capacity);
				
			}
			
			
			Log.writeInLogFile(this.getName()," Free capcacity:"+free_capacity);
			
//			/* Send an ACK to Broker*/
//			if(this.storedData.get(tuple.getTupleType()).contains(tuple)){
//				System.out.println(this.getName()+ " Tuple has been well stored:"+tuple.toString()+"\n");
//				Log.writeInLogFile(this.getName(), "Tuple has been well stored:"+tuple.toString()+"\n");
...
//				Log.writeInLogFile(this.getName(), "Tuple does not been well stored:"+tuple.toString());
//				Application app = CloudSim.broker.getAppliaction();
...
//			}			
			...
			Log.writeInLogFile(this.getName(), " this is not the first stored tuple:"+tuple.toString());
...
//		//**String str = sc.nextLine();
	...
	private double archiveAllOldVersionOfAllTuples(double free_capacity) {
		// TODO Auto-generated method stub
		//**System.out.println(this.getName()+": archive All Old Version Of All Stored Tuples");
		//printAllStoredData();
		
		for(String tupleType : this.storedData.keySet()) {
			//**System.out.println("tupleType:"+tupleType);
			StoredDataQueue storedVersions = this.storedData.get(tupleType);
			
			int size = storedVersions.size();
			//**System.out.println("Size ="+size);
			
			for(int i=0;i<size-1;i++) {
				Tuple tuple = storedVersions.iterator().next();
				storedVersions.remove(tuple);
				//**System.out.println("delete old version:"+tuple+toString());
				free_capacity += tuple.getCloudletFileSize();
			}
		}
	...
	private void sendPeriodicTuple(SimEvent ev) {
		...
		Log.writeInLogFile(this.getName(), "send periodic tuple:"+edge.getTupleType()+"\tmodule:"+srcModule);
		...
		if(module == null){
			...
			Log.writeInLogFile(this.getName(), "module:"+srcModule+"\tis not allocated in this node!");
			return;
		}
...
//				LatencyStats.add_Overall_Letency(LatencyStats.getOverall_Latency()+delay*nb_Unit);
				...
				System.out.println("error, send data to actuator");
				System.exit(0);
				return;
			}
		}
...
//		}
	...
	////*//**System.out.println(this.getName()+" processTupleProcess: process tuple and send vm-data-center to self for :"+tuple.getTupleType());
		Log.writeInLogFile(this.getName(), "processTupleProcess: process tuple and send vm-data-center to self for :"+tuple.getTupleType());
		...
		////*//**System.out.println(this.getName()+" Send a TUPLE_RETRIEVE_ACK to the broker for tuple:"+tuple.toString());
			Log.writeInLogFile(this.getName(), "Send a TUPLE_RETRIEVE_ACK to the broker for tuple:"+tuple.toString());
		...
		//if(appToModulesMap.get(tuple.getAppId()).contains(tuple.getDestModuleName().get(0))){
			...
			if(destDevId != getId()){
				Log.writeInLogFile(this.getName(), "Error! Tuple destination module is not deployed in this entity! ->"+destDevId+"   !=   "+getId());
...
//		}
	...
	protected void sendUpFreeLink(Tuple tuple){
		...
		Log.writeInLogFile(this.getName(), ": Send to broker for Storage, tuple:"+tuple.toString());
		float latency = 0;
		...
		//Mise à jour de la pénalité
		PenalityStats.add_Overall_write_penalityIoT(PenalityStats.penalityIoT(latency ));
		PenalityStats.add_penality_per_consumer(2, tuple.getTupleType(), PenalityStats.penalityIoT(latency));
	}
	...
	public void printAllStoredData(){
		...
		Log.writeInLogFile(this.getName(),"Print All data stored at "+ this.getName() );
		if(storedData.keySet() != null){
			System.out.println(this.getName()+" Stores:");
			Log.writeInLogFile(this.getName()," Stores:");
			for(String tupleTupe : storedData.keySet()){
				...
				while(itr.hasNext()){
					...
					Log.writeInLogFile(this.getName(),data.toString());
				}
			}	
		}
	}
