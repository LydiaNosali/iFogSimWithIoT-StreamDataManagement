...
public abstract class SimEntity implements Cloneable {
	...
	public void run() {
		...
		//Log.writeInLogFile(this.getName(),"\trun in entity:"+this.getName());

		while (ev != null) {
			Log.writeInLogFile(this.getName(),"\tProcessing ev of evbuf:"+ev.toString());
			//System.out.println(this.getName()+"\tProcessing ev of evbuf:"+ev.toString());
