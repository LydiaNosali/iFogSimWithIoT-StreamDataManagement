...
public class MakeLPFile{
	public  static int nb_DataHost; //N
	public  static int nb_DataProd; //L
	public  static int nb_DataCons; //M
	
	public  static int base; //b
	
	private static final int nb_thread = 32;
	
	public  static float [][] write_basis_laenty; //TS ts
	public  static float [][] read_basis_laenty; //TR tr
	
	public  static int [][] consProd_matrix; //AP abonnement a
	
	public  static int [][] popularity; //NR nombre de requetes conso_donnee
	
	public  static double [] freeCapacity; //FS capacité de stockage libre existant partie stockage
	
	public  static float [] dataSize; // DS
	...
	private boolean contructionLpFile(int nb_GW) throws IOException, InterruptedException{
		if(checkSolution()){
			...
			if(DataPlacement.Strategy.equals("iFogStorSLA")) {
				equation(nb_GW);
			}else {
				equation(nb_GW);
			}
	...
	private void equation(int nb_GW) throws IOException, InterruptedException{
		...
		if(nb_DataProd<nb_thread*nb_thread){
			...
			double [][] fac = new double [nb_DataProd][nb_DataHost];
			...
			factors=fac;
//			for(int i=0;i<MakeLPFile.nb_DataProd;i++){
...
//			}
	...
	private void equation2(int nb_GW) throws IOException, InterruptedException{
		FindFactorsThread2 thrd;
		...
		if(nb_DataProd<nb_thread*nb_thread){
			...
			int begin=0;
			int end=nb_DataProd;
			thrd = new FindFactorsThread2("1", begin, end);
			thrd.start();
			////System.out.println("Thread "+1+"  is started!");
			thrd.join();
			////System.out.println("Thread "+1+"  is joined!");
			...
			int plane = (int)nb_DataProd/(int)nb_thread;
			FindFactorsThread2 [] threadTable =  new FindFactorsThread2[nb_thread];		
			...
			for(int nb_thrd=0;nb_thrd<nb_thread;nb_thrd++){
				...
				thrd = new FindFactorsThread2(String.valueOf(nb_thrd), begin, end);
				threadTable[nb_thrd] = thrd;
				thrd.start();
...
//			}
	...
	private void constraints(int nb_GW) throws IOException{
		...
		try {
			...
			//Contrainte1
			//chaque donnée doit être stockée dans un et un seul hebergeur
			for (int i = 0; i < nb_DataProd;i++){
				fw.write("  ");
		 		for (int j = 0; j < nb_DataHost; j++){
		 			fw.write("x"+i+"_"+j);		 
			...
			//Contrainte2
			//La quantité de donnée mise dans un hebergeur ne doit pas dépasser la capacité restante
			for (int j = 0; j < nb_DataHost;j++){
				fw.write("  ");
		 		for (int i = 0; i < nb_DataProd; i++){
	...
	private void bounds(int nb_GW) throws IOException{
		...
		try {
			//Contrainte3
			//force les variables à être des variables de décision 1/0
			BufferedWriter fw = new BufferedWriter(lpFile);
			fw.write("Bounds\n");
	...
	private void initializePopularity(int nb_GW) throws FileNotFoundException{
		FileReader fichier = new FileReader(nb_GW+"consProdPopularity_"+DataPlacement.nb_DataCons_By_DataProd+".txt");
		int [][] consProdPopularity = new int [nb_DataProd][nb_DataCons];
		BufferedReader in = null;
		try{
			in = new BufferedReader (fichier);
			String line =null;
			...
			while((line = in.readLine()) != null){
				...
				int col = 0;
				for(String val : splited){
					consProdPopularity[row][col] = Integer.valueOf(val);
					col++;
				}
				row++;
			}
			in.close();
			popularity = consProdPopularity;
		} catch (FileNotFoundException e) {
			e.printStackTrace();
		} catch (IOException e) {
			e.printStackTrace();
	...
	private void initializeFreeCapacity(int nb_GW) throws FileNotFoundException{
		...
		double [] free = new double [nb_DataHost];
		BufferedReader in = null;
		try{
			in = new BufferedReader (fichier);
			String line = in.readLine();
