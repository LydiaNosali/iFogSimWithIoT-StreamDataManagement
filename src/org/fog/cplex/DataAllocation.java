...
public class DataAllocation {
	...
	public static Map<String, Integer> intial_cachePlacementMap = new HashMap<String, Integer>();
	public static Map<Integer, String> time_Stream_consumption = new HashMap<Integer, String>();
	public static Map<String, List<Integer>> stream_consumersList = new HashMap<String, List<Integer>>();
	...
	public static void printCachePlacementMap() {
		System.out.println("printCachePlacementMap");
		for(String tupletype : cachePlacementMap.keySet()) {
			System.out.println(tupletype+" is stored at "+cachePlacementMap.get(tupletype).toString());
		}
	}
	
	public static void processCacheAdd(int nodeId, String tupleType) {
		// TODO Auto-generated method stub
		//Log.writeInLogFile(this.getName(), "Cache Add:"+tupleTyp);
		List<Integer> cacheList;
		if (cachePlacementMap.containsKey(tupleType)) {
			if (!cachePlacementMap.get(tupleType).contains(nodeId)) {
				cachePlacementMap.get(tupleType).add(nodeId);
			}
		}
		else {
			cacheList= new ArrayList<Integer>();
			cacheList.add(nodeId);
			cachePlacementMap.put(tupleType, cacheList);
		}
		//Log.writeInLogFile(this.getName(), "CacheList ="+cachePlacementMap.get(tupletype));
		
	}
	
	public static void processCacheRemove(int sourceEdge, String tupleType) {
		// TODO Auto-generated method stub
		...
		if (!cachePlacementMap.get(tupleType).contains(sourceEdge)) {
			//Log.writeInLogFile(this.getName(), "Erreur src non existant dans la liste de cache ="+cachePlacementMap.get(tupletype));
			System.out.println("Erreur sourceEdge non existant dans la liste de cache ");
			System.exit(0);
		}
		
		if(cachePlacementMap.get(tupleType).size()==1)
			cachePlacementMap.remove(tupleType);
		else
			cachePlacementMap.get(tupleType).remove(new Integer(sourceEdge));
		
		//Log.writeInLogFile(this.getName(), "CacheList ="+cachePlacementMap.get(tupletype));
	}
	
	
	public List<Integer> getEmplacementNodeIdCache(String tupleType) {
		// TODO Auto-generated method stub
		if(!cachePlacementMap.containsKey(tupleType)) {
			System.out.println("Error, data  "+tupleType+" doesn't exist, data allocation");
			System.exit(0);
		}
		return cachePlacementMap.get(tupleType);
	}
