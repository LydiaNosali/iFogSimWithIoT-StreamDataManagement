...
public class Application {
	...
	public Map<String, List<String>> mapSelectivity = new HashMap<String,List<String>>();
	
	public Map< Integer, String> fogDevicetotTupleMap = new HashMap<Integer, String>();
	
	public Map<String, Integer> tupleToFogDeviceMap = new HashMap<String, Integer>();
	
	public int total_nb_cons =0;
	
	private DataConsistency dataConsistencyProtocol;
	
	public Map<String, String> DataConsistencyMap = new HashMap<String, String>();

	
	public void setDataConsistencyMap(List<Integer> criticals){
		
		if(criticals == null){
			for(int i=0;i<tuples.size();i++){
				DataConsistencyMap.put(tuples.get(i), DataPlacement.Weak);
			}
		}else {
			for(int i=0;i<tuples.size();i++){
				if(criticals.contains(i)){
					DataConsistencyMap.put(tuples.get(i), DataPlacement.Strong);
				}else {
					DataConsistencyMap.put(tuples.get(i), DataPlacement.Weak);
				}
			}
	 ...
	 * @param ram
	 */
	public void addAppModule(String moduleName, int ram, int mips, double size, long bw){
//		int mips = 1000;
//		long size = 10000;
//		long bw = 1000;
		String vmm = "Xen";
	...
	public void addAppEdge(AppEdge edge){
		getEdges().add(edge);
		getEdgeMap().put(edge.getTupleType(), edge);
	}
...
//	public void addServicesToApplication(List<FogDevice> listOfFogDevices) {
//		System.out.println("Add services to application");
//		for(FogDevice fogdev : listOfFogDevices){
//			if(fogdev.getName().startsWith("DC")){
//				int i = Integer.valueOf(fogdev.getName().substring(2));
//				addAppModule("serviceDC" + (int) i, DataPlacement.SERVICE_DC_RAM, DataPlacement.SERVICE_DC_MIPS, fogdev.getVmAllocationPolicy().getHostList().get(0).getStorage(), DataPlacement.SERVICE_DC_BW);
//			}else if(fogdev.getName().startsWith("RPOP")){
//				int i = Integer.valueOf(fogdev.getName().substring(4));
//				addAppModule("serviceRPOP" + (int) (i), DataPlacement.SERVICE_RPOP_RAM, DataPlacement.SERVICE_RPOP_MIPS, fogdev.getVmAllocationPolicy().getHostList().get(0).getStorage(), DataPlacement.SERVICE_RPOP_BW);
//
//			}else if(fogdev.getName().startsWith("LPOP")){
//				int i = Integer.valueOf(fogdev.getName().substring(4));
//				addAppModule("serviceLPOP" + (int) (i), DataPlacement.SERVICE_LPOP_RAM, DataPlacement.SERVICE_LPOP_MIPS, fogdev.getVmAllocationPolicy().getHostList().get(0).getStorage(), DataPlacement.SERVICE_LPOP_BW);
//
//			}else if(fogdev.getName().startsWith("HGW")){
//				int i = Integer.valueOf(fogdev.getName().substring(3));
//				addAppModule("serviceHGW" + (int) (i), DataPlacement.SERVICE_HGW_RAM, DataPlacement.SERVICE_HGW_MIPS, fogdev.getVmAllocationPolicy().getHostList().get(0).getStorage(), DataPlacement.SERVICE_HGW_BW);
//
//			}else{
//				System.out.println("Error in addServicesToApplication! there is no entity:"+fogdev.getName());
//				System.exit(0);
//			}
//		}
//}
	...
	public void addServicesToApplication(List<FogDevice> listOfFogDevices) {
		...
		for(FogDevice fogdev : listOfFogDevices){
			if(fogdev.getName().startsWith("DC")){
				int i = Integer.valueOf(fogdev.getName().substring(2));
				addAppModule("serviceDC" + (int) i, DataPlacement.SERVICE_DC_RAM, DataPlacement.SERVICE_DC_MIPS, 0, DataPlacement.SERVICE_DC_BW);
			}else if(fogdev.getName().startsWith("RPOP")){
				int i = Integer.valueOf(fogdev.getName().substring(4));
				addAppModule("serviceRPOP" + (int) (i), DataPlacement.SERVICE_RPOP_RAM, DataPlacement.SERVICE_RPOP_MIPS, 0, DataPlacement.SERVICE_RPOP_BW);

			}else if(fogdev.getName().startsWith("LPOP")){
				int i = Integer.valueOf(fogdev.getName().substring(4));
				addAppModule("serviceLPOP" + (int) (i), DataPlacement.SERVICE_LPOP_RAM, DataPlacement.SERVICE_LPOP_MIPS, 0, DataPlacement.SERVICE_LPOP_BW);

			}else if(fogdev.getName().startsWith("HGW")){
				int i = Integer.valueOf(fogdev.getName().substring(3));
				addAppModule("serviceHGW" + (int) (i), DataPlacement.SERVICE_HGW_RAM, DataPlacement.SERVICE_HGW_MIPS, 0, DataPlacement.SERVICE_HGW_BW);
	...
	public void addTupleMappingFraction2(){
		...
		try {
			...
			for (int i = 0; i < DataPlacement.nb_Service_HGW; i++) {
				...
				addTupleMapping("serviceHGW" + i, "TempSNR"+ (int) i * DataPlacement.nb_SnrPerHGW, "TempHGW" + i, new FractionalSelectivity(DataPlacement.snrFraction));
				/* Tuple from SNR -- HGW-- ACT */
				for (int actIndex = 0; actIndex < DataPlacement.nb_ActPerHGW; actIndex++)
					addTupleMapping("serviceHGW" + i, "TempSNR"+ (int) i * DataPlacement.nb_SnrPerHGW, "TempAct"+ (int) (actIndex + i * DataPlacement.nb_ActPerHGW),new FractionalSelectivity(0.0));
			}
			...
			for (String inputTuple : getEdgeMap().keySet()) {
				List<String> destinationServices = new ArrayList<String> (getEdgeMap().get(inputTuple).getDestination());
				...
				for (String destService : destinationServices) {
					...
					if (destService.startsWith("serviceLPOP") || destService.startsWith("serviceRPOP")) {
						...
						if (selectivityMap.size() < 1 && !inputTuple.equals(outputTuple)) {
							addTupleMapping(mod.getName(),inputTuple, outputTuple,new FractionalSelectivity(DataPlacement.serviceFraction));
	...
	public void addTupleMappingFraction() {
		...
		try {
			...
			for(AppModule mod: this.getModules()){
				String modName  = mod.getName();
				
				if (modName.startsWith("serviceLPOP") || modName.startsWith("serviceRPOP")) {
					String outputTuple = "Temp" + modName.substring(7);
					
					for(AppEdge edge: edges){
						List<String> destinationServices = new ArrayList<String> (edge.getDestination());
						
						if(destinationServices.contains(modName)){
							
							if(modName.equals(edge.getSource())){
								continue;
							}

							String inputTuple = "Temp" + edge.getSource().substring(7);
		
								
							Map<Pair<String, String>, SelectivityModel> selectivityMap = mod.getSelectivityMap();
							if(selectivityMap.isEmpty()){
								addTupleMapping(mod.getName(),inputTuple, outputTuple,new FractionalSelectivity(DataPlacement.serviceFraction));
...
//		int nb_cons = (int) (Math.random() * DataPlacement.nb_DataCons_By_DataProd +1);
		int nb_cons = DataPlacement.nb_DataCons_By_DataProd ;
		total_nb_cons += nb_cons;
		
		if(nb_cons==0){
			System.out.println("nb_cons=0 in Application");
			System.exit(0);
		}
...
//					}
	...
	public void saveApplicationEdges()
			throws IOException {
		// TODO Auto-generated method stub
	...
	public void initializeTupeToFogDeviceIdMap(){
		...
		for(AppEdge edge : edges){
			...
			String sourceModule = edge.getSource();
			
			int sourceDevId = getFogDeviceByName(ModuleMapping.getDeviceHostModule(sourceModule)).getId();
			
			fogDevicetotTupleMap.put(sourceDevId, tupleType);
			tupleToFogDeviceMap.put(tupleType, sourceDevId);
		}
	...
	public void printModulesSelectivityMap(){
		...
		for(String key: mapSelectivity.keySet()){
			System.out.println("key:"+key+"\tvalue:"+mapSelectivity.get(key));
		}
	}
	
	public DataConsistency getDataConsistencyProtocol(){
		return this.dataConsistencyProtocol;
	}

	public void setDataConsistencyProtocol(DataConsistency dataConsistencyProtocol) {
		// TODO Auto-generated method stub
		this.dataConsistencyProtocol = dataConsistencyProtocol; 
	}

	public void resetStoredData() {
		// TODO Auto-generated method stub
		for (FogDevice fogdev: fogDevices) {
			fogdev.storedData.clear();
			fogdev.requestId = 0;
			fogdev.lastVersionMap.clear();
		}
	}
